...
struct NumericToStringCastFunctor {
  ...
  static Status Convert(KernelContext* ctx, const ArrayDataBase& input, ExecArrayData* output) {
    FormatterType formatter(input.Type().Clone());
    BuilderType builder(input.Type().Clone(), ctx->memory_pool());
    ...
    *output = ExecArrayData(*output_array->data());
    return Status::OK();
  }
};
...
struct TemporalToStringCastFunctor<O, TimestampType> {
  ...
  static Status Convert(KernelContext* ctx, const ArrayDataBase& input, ArrayData* output) {
  ...
  static Status ConvertZoned(const ArrayDataBase& input, const std::string& timezone,
                             BuilderType* builder) {
...
Status CastBinaryToBinaryOffsets(KernelContext* ctx, const ArrayDataBase& input,
                                 ArrayDataBase* output) {
...
BinaryToBinaryCastExec(KernelContext* ctx, const ExecBatch& batch, Datum* out) {
  ...
  const int32_t out_width =
      checked_cast<FixedSizeBinaryType *>(options.to_type.get())->byte_width();
